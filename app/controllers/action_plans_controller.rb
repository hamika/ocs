class ActionPlansController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @action_plan.update(action_plan_params)
        format.html { redirect_to @action_plan,
          notice: 'Action plan was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to action_plans_url,
        notice: 'Action plan was successfully destroyed.' }
  ...
  def action_plan_params
    params.require(:action_plan).permit(:term_from, :term_to,
      :target, :achievement, :body)
  end
end
